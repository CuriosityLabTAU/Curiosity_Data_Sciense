...
from tensorflow.keras.layers import Input, Dense, Dropout, Activation, BatchNormalization, MaxPooling2D, LSTM, Flatten, Lambda, Subtract, Multiply, Add, concatenate
...
K.clear_session()
experiment= str(experiment_name)+ '_' + str(file) +'_DQN_' + str(discount_rate) + '_' + str(iterations)


number_of_experiment=30
...
class Agent:
    ...
    def create_Q_model(self):
        ...
        output = Dense(self.output_dim,  kernel_initializer='VarianceScaling', activation='linear', name='output')(x) # Ones
    ...
    def act(self, state, policy_calc, eps, selected_actions):  
        ...
        if policy_calc == 0:
            ...
            if np.random.rand() < eps :  
                ...
                if len(avilable_actions_Q_threshold[avilable_actions_Q_threshold>internal_threshold])>0:#self.threshold
            ...
            if len(avilable_actions_Q_threshold[avilable_actions_Q_threshold>value_threshold])>0:
                a = np.argmax(avilable_actions_Q_threshold)
                a_Q = Q_values[a]
            else:
                a = -999
    ...
    def is_done(self, a_Q, selected_actions): ### need to fix 
        ...
        if a_Q < internal_threshold or sum(selected_actions == 1) == self.input_dim: #self.threshold
            done = 1
        return done 
...
#         return self.model.train_on_batch(X_batch, y_batch)
        return self.full_model.fit(X_batch, y_batch, epochs=5, steps_per_epoch=10, batch_size=batch_size, verbose=0)#, callbacks=[csv_logger])
...
def create_batch(memory, discount_rate):
    ...
    discount_future_rewards = 0 * tf.reduce_max(future_rewards*available_actions, axis=1)
...
#         print('thread_id {} , s {}'.format(thread_id, s))
    ...
    episode_memory = np.asarray(memory)
    if len(episode_memory) >0 :
        errors = episode_memory[:, 2].astype(np.float)
...
##########################Experiment setup##########################
    ...
    df = pd.DataFrame(columns=('episode','policy_columns','policy_accuracy_train','policy_accuracy_test'))
    ...
    for epi in range(episodes):
        ...
        if len(episode_memory_store)>0:
            ...
            # Calculate policy train accuracy    
                ...
                policy_X_train_main, policy_X_test_main, policy_y_train, policy_y_test =  env.data_split(policy_X,policy_y)
                policy_accuracy_train = env.accuracy(policy_selected_actions, policy_X_train_main, policy_X_test_main, policy_y_train, policy_y_test)
...
#     print('policy test accuracy: {} '.format(policy_accuracy_test))
    ...
    if not os.path.exists('Experiments/'+ str(experiment) + '/experiment_dict.pickle'):
        os.mknod('Experiments/'+ str(experiment) + '/experiment_dict.pickle')
        experiment_dict= {}
        ...
        with open('Experiments/'+ str(experiment) + '/experiment_dict.pickle', "rb") as handle:
