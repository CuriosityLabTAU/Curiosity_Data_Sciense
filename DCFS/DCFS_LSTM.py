...
K.clear_session()
experiment= str(experiment_name)+ '_' + str(file) +'_lstm_' + str(discount_rate) + '_' + str(iterations)
...
test_file_name = file +'_Test_Data.csv'
...
class actorthread(threading.Thread):
    ...
    def run(self, env, agent, episode_size, eps):
        ...
        episode_memory_store = run_process.runprocess(env, agent, episode_size, eps, discount_rate)
        if np.array(episode_memory_store).shape[0]>0:
            ...
            for episode_memory in episode_memory_store:
                X_batch,a_batch, y_batch = genreal_func.create_batch(np.array(episode_memory),discount_rate, agent)
...
#     agent.updae_model_for_infereance_weights()
    ...
    episodes= int(np.floor(max(len(env.action_space),10)/10)*iterations/final_threads)
...
#         a_for_train= a_for_train.reshape(a_for_train.shape[0], a_for_train.shape[2],a_for_train.shape[3])
        ...
        agent.train([np.array(x_memory), np.array(a_memory).reshape(np.array(a_memory).shape[0],np.array(a_memory).shape[1],1)]
                    , np.array(y_memory).reshape(np.array(y_memory).shape[0],np.array(y_memory).shape[2], np.array(y_memory).shape[1]
        ...
        while not policy_done:
            ...
            if policy_a_Q >= agent.external_threshold:
                ...
                policy_done = agent.is_done(policy_a_Q, policy_selected_actions, policy_calc=1)
                policy_s = policy_s2
            else: 
                policy_done = 1
...
#        df=df.append({'episode':str(epi+1), 'policy_columns':str(policy_columns), 'policy_order':str(policy_order), 'policy_accuracy_train':policy_accuracy_train,'policy_accuracy_test':policy_accuracy_test}, ignore_index=True)
