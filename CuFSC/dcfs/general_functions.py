...
class genreal_func:
    ...
    def create_batch(self, memory, discount_rate, agent):
        ...
        r = sample[:, 2].astype(np.float)
        s2 = sample[:, 3]
        d = sample[:, 4] * 1.
        available_actions = sample[:, 5]  ##added available_actions
        available_actions = np.vstack(available_actions)
        available_actions = available_actions.reshape(1, available_actions.shape[0], available_actions.shape[1])        
        
        X_batch = np.vstack(s)
        X_batch = X_batch.reshape(1, X_batch.shape[0], X_batch.shape[1])
        ## padding with 0 in the begining 
        X_batch = np.pad(X_batch, [(0, 0), ((X_batch.shape[2] - X_batch.shape[1]), 0),(0, 0)], mode='constant')        
