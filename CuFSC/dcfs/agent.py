...
class Agent:
    def __init__(self, internal_threshold, external_threshold, rnn_unit, number_of_features_in_data):
#         self.csv_logger = CSVLogger('log.csv', append=True, separator=';')
        self.input_dim = number_of_features_in_data
        self.output_dim = number_of_features_in_data
...
#         Q_values = self.predicting_q_model.predict(state, verbose=0).flatten()
        Q_values = self.predicting_q_model(state)[0][0]
...
#         return self.target_model.predict(X_batch, verbose=0)
        return self.target_model(X_batch)[0][0]
